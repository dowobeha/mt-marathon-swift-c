...
print(observed_y.reduce(0.0, combine: -))
...
if let cublas = CUBLAS() {
	...
	if let cuda_vector_x = CudaVector(count:number_of_data_points), 
	       cuda_vector_y = CudaVector(count:number_of_data_points),
	       cuda_vector_error = CudaVector(count:number_of_data_points),
	       cuda_vector_squared_error = CudaVector(count:number_of_data_points) {

		cuda_vector_x.copyDataToDevice(observed_x)
		cuda_vector_y.copyDataToDevice(observed_y)
		
		while (!(isinf(mean_squared_error) || epsilon > mean_squared_error)) {
			...
			let error_sum_for_y_intercept = cuda_vector_error.copyDataFromDevice()!.reduce(0.0, combine: +)
			...
			cuda_vector_squared_error.copyDataWithinDevice(cuda_vector_error)
			...
			let squared_error_sum = cublas.asum(cuda_vector_squared_error)!
			...
			let error_sum_for_slope = cublas.sum(cuda_vector_error)!
